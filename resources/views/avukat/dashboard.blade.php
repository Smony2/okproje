...
@section('title')
    <title>Avukat Paneli | Adliye Dijital Takip Sistemi</title>
@endsection
                                ...
                                <div class="d-flex align-items-center">
                                    <div class="w-64-px h-64-px radius-16 bg-base-50 d-flex justify-content-center align-items-center me-20">
                                        <span class="mb-0 w-40-px h-40-px bg-primary-600 flex-shrink-0 text-white d-flex justify-content-center align-items-center radius-8 h6 mb-0">
                                            <iconify-icon icon="gridicons:multiple-users" class="icon"></iconify-icon>
                                        </span>
                                    </div>
                                    <div>
                                        <span class="mb-2 fw-medium text-secondary-light text-md">Yorum Yapılan Katip</span>
                                        <h6 class="fw-semibold my-1">{{ $yorumYapilanKatip }}</h6>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="row gy-4">
            ...
            <div class="col-xl-8">
                <div class="row gy-4">
                                        ...
                                        @forelse($islerim as $is)
                                            <tr>
                                                ...
                                                <td data-label="Durum">
                                                    @php
                                                        $durumMap = [
                                                            'bekliyor' => 'status-btn ongoing',
                                                            'devam ediyor' => 'status-btn waiting',
                                                ...
                                                <td data-label="Detay">
                                                    <a href="{{ route('avukat.isler.detay', $is->id) }}" class="w-32-px h-32-px bg-primary-light text-primary-600 rounded-circle d-inline-flex align-items-center justify-content-center">
                        ...
                        <div class="card modern-chart-card">
                            <div class="card-header">
                                <div class="d-flex align-items-center justify-content-between">
                                    <h6 class="mb-0 fw-bold text-lg">
                                        <iconify-icon icon="solar:wallet-money-bold" class="me-2 text-danger"></iconify-icon>
                                        Harcama Trendi
                                    </h6>
                                    <div class="earnings-summary">
                                        <span class="earning-total" style="background: #fee2e2; color: #dc2626;">
                                            Toplam: {{ number_format(collect($harcamaTrendi)->sum('harcama'), 0) }} Jeton
                                        </span>
                                    </div>
                                </div>
                            </div>
                            <div class="card-body">
                                <div class="modern-chart-container">
                                    <canvas id="harcamaTrendiChart"></canvas>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Katip Dağılımı - Full Width -->
                    <div class="col-12">
                        <div class="card modern-chart-card">
                            <div class="card-header">
                                <h6 class="mb-0 fw-bold text-lg">
                                    <iconify-icon icon="solar:users-group-two-rounded-bold" class="me-2 text-info"></iconify-icon>
                                    Katip Bazında İş Dağılımı %
                                </h6>
                            </div>
                            <div class="card-body">
                                <div class="modern-chart-container small">
                                    <canvas id="katipChart"></canvas>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="card-header">
                                <div class="d-flex align-items-center flex-wrap gap-2 justify-content-between">
                                    <h6 class="mb-0 fw-bold text-lg">
                                        ...
                                        Son Bildirimler
                                        @if(isset($okunmamisBildirimSayisi) && $okunmamisBildirimSayisi > 0)
                                            <span class="badge bg-danger ms-2">{{ $okunmamisBildirimSayisi }}</span>
                                        @endif
                                    </h6>
                        ...
                        <div class="card modern-chart-card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="chart-legend-list mt-3">
                                    @foreach($isTuruDagilimi as $index => $tur)
                                        <div class="legend-list-item">
                                            <span class="legend-color" style="background: {{ ['#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4'][$index % 6] }};"></span>
                                            <span class="legend-text">{{ $tur->islem_tipi }}</span>
                                            <span class="legend-value">{{ $tur->adet }}</span>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
...
@section('jsler')
    ...
    <script>
        ...
        // 3. Katip Horizontal Bar Chart
        const katipCtx = document.getElementById('katipChart').getContext('2d');
        const katipData = {!! json_encode($katipDagilimi->pluck('is_sayisi')->toArray()) !!};
        const katipLabels = {!! json_encode($katipDagilimi->pluck('katip_adi')->toArray()) !!};
        ...
        // 7. Harcama Trendi Chart
        const harcamaCtx = document.getElementById('harcamaTrendiChart').getContext('2d');
        const harcamaData = {!! json_encode($harcamaTrendi) !!};

        const harcamaGradient = harcamaCtx.createLinearGradient(0, 0, 0, 300);
        harcamaGradient.addColorStop(0, 'rgba(239, 68, 68, 0.6)');
        harcamaGradient.addColorStop(1, 'rgba(239, 68, 68, 0.1)');

        new Chart(harcamaCtx, {
            type: 'line',
            data: {
                labels: harcamaData.map(k => k.ay),
                datasets: [{
                    label: 'Harcama (Jeton)',
                    data: harcamaData.map(k => k.harcama),
                    borderColor: '#ef4444',
                    backgroundColor: harcamaGradient,
                    borderWidth: 4,
                    fill: true,
                    tension: 0.4,
                    pointRadius: 8,
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            label: function(context) {
                                return `Harcama: ${context.parsed.y.toLocaleString('tr-TR')} Jeton`;
                            }
                        }
                    }
                },
                    ...
                    y: {
                        ...
                        ticks: {
                            ...
                            callback: function(value) {
                                return value.toLocaleString('tr-TR') + ' Jeton';
                            }
                        }
                    },
                    x: {
